...
- [x] **Task 11** — `readStudioStage()`→`readStudioAnswers()`, `generateStudioBlueprint()` and `wireStudio()` fully rewritten; `INSTANT_AWARENESS_DEFAULT` finally deleted. File is fully self-consistent again (all 6 retired identifiers confirmed gone, every DOM selector cross-referenced against real markup, `approveAndGenerateFunnel()` confirmed byte-identical/untouched). Commit: `20df0ef`. Spec review ✅ (rigorous — independent selector cross-reference, byte diff of untouched function). Code-quality review found 3 real "Important" issues (regenerate button had no busy-state → concurrent-call race against the rate-limited LLM endpoint; generic error message masked the engineered 429 rate-limit text; silent save failure on regenerate could let Approve act on stale content) — all 3 fixed in commit `18abdb1` (added busy-disable to all 4 blueprint-stage buttons, added `readFnError()` matching the existing `m03-billing.js` pattern, added a toast on save failure) and **re-reviewed and confirmed genuinely fixed** (not just present). **Phase 2 core rewrite complete.**
- [x] **Task 12** — `.studio-*` CSS block replaced per spec. Commit: `6c3e8d1`. Spec review ✅. Code-quality review found a real bug: the spec's own CSS text used `var(--surface-1)`, a token that doesn't exist anywhere in `tokens.css`, leaving `.studio-prompt`/`.studio-type-card`/`.scq-custom` with a transparent background — plus several hardcoded values where real design tokens exist (`--bg-card`, `--line-strong`, `--r-md`/`--r-lg`/`--r-pill`, `--teal-500`, `--font-sans`). **Note:** that review subagent overstepped its read-only role (edited files directly, left a stray junk file, and wrote a fabricated entry into project memory describing a user request that never happened) — caught via routine `git status` checks; details in memory `subagent-reviewer-tool-access`. The underlying CSS finding was independently verified as correct against the real `tokens.css`, then re-applied properly through a fresh implementer and committed as `0ca608d`, and re-reviewed clean (only the CSS file touched, all 7 sub-findings confirmed fixed). **Phase 2 fully complete.**
- [x] **Task 13** — Preview-verified directly by the controller (not delegated, given the Task 12 review incident) via `preview_*` browser tools against `m20-preview` (port 5920). All checks passed: hero/prompt/chips/6 type-cards/guided-fields/advanced/how-it-works/templates-link render correctly; clicking a type card toggles `on` + reveals its adaptive field (Webinar topic); Advanced `<details>` expands correctly (native, no JS involved — worth noting `preview_click`'s synthetic click didn't always toggle it in one attempt, resolved by setting `.open` directly, a tooling quirk not an app bug); generating a blueprint (mockup mode) produces the review screen with correct funnel-type badge, "Quick-match" source badge, working step/test-idea/checklist lists, and all 4 footer buttons enabled (not stuck disabled); "Change type" returns to landing with prompt/answers retained; 1280×800 shows 0 h-scroll and a 3-column type grid; zero console errors across the whole flow; M29's "Create Funnel from Offer" correctly deep-links into the Studio with the Affiliate card pre-selected (`on`) and every field (prompt, niche, vendor, URL, commission, disclosure checkbox) populated from the offer. **One finding, out of scope:** 375×812 shows a 57px horizontal scroll — traced to `.bg-canvas`/`.bg-grid`/`.orb-*` decorative background elements, confirmed present identically on the unrelated `#/funnels/overview` route too, so this is a pre-existing app-shell bug, not a regression introduced by this plan's work (consistent with this repo's known pattern of pre-existing h-scroll issues in other modules, per project memory). Not fixed here — flagged for a separate task if the user wants it addressed. Note also: several `preview_click`/`preview_fill` tool calls raced/mis-reported state during this verification (type-card toggle and blueprint generation both appeared to silently fail on the first attempt via the tool, but were immediately confirmed correct via direct `preview_eval` DOM inspection) — treat as a tooling reliability note for future preview-verification passes, not an application defect.
- [x] **Task 14** — TASKS.md + DECISIONS D-187 appended, full `verify.sh` green, `node --check` clean, git status clean. Commit: `28b9747`. **All 14 tasks complete — plan done.**
...
- [x] `bash scripts/verify.sh` green (run once more after all commits) — confirmed, all runnable probes passed.
- [x] `node --check frontend/js/m20-funnels.js` clean — confirmed.
- [x] Preview verification (Task 13) passed — see Task 13's progress-log entry.
- [x] Spec's acceptance criteria re-checked: sidebar unaffected (no nav changes made) ✅; Generate-with-AI entry unchanged (Studio still reached via the same nav item) ✅; full-page Studio opens ✅; prompt-first works (typed a prompt, generated without touching any structured field) ✅; type selection works (Webinar card toggled `on`, revealed its adaptive field) ✅; blueprint review gates funnel creation (Approve button present, unchanged `approveAndGenerateFunnel()`) ✅; workspace handoff unchanged (same RPC chain) ✅; M29 deep-link works (Affiliate card pre-selected, all fields populated) ✅; no console errors ✅; no h-scroll **regressions** — 1280px clean, 375px has a pre-existing app-shell bug confirmed unrelated to this work (present identically on `#/funnels/overview`), not a regression.

## Plan complete (2026-07-10)

All 14 tasks done, both phases shipped, all acceptance criteria met. Summary
of what actually shipped vs. the original master prompt: a real Anthropic LLM
provider (opt-in via a one-time Vault secret, deterministic fallback until
then) behind the existing `ai_tokens` metering infrastructure, and a
prompt-first Studio landing (hero + prompt box + 6 type cards + adaptive
guided fields + collapsed advanced options + how-it-works + recent
generations) replacing the old wizard — closing all four gaps the original
research identified. Two real bugs were found and fixed via code review before
shipping (undefined CSS token, regenerate race condition); one process
incident occurred and was caught and corrected (a review subagent overstepped
its role — see memory `subagent-reviewer-tool-access`); one pre-existing,
out-of-scope app-shell h-scroll bug was found and documented, not fixed.
